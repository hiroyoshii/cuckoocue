...
try {
  ...
  const androidPage = androidBrowser.contexts()[0].pages().find((candidate) => candidate.url().startsWith(`${base}/?run_id=${runId}#edit_tasks=`));
  assert.ok(androidPage, "Android completion button must open the same Run in Chrome");
  const incomingEditorUrl = androidPage.url();
  assert.deepEqual(new URLSearchParams(new URL(incomingEditorUrl).hash.slice(1)).get("edit_tasks").split(",").sort(), before.tasks.map((task) => task.id).sort());
  ...
  const nativeHistoryResponse = androidPage.waitForResponse((response) => response.url() === `${base}/api/runs/${runId}?completed_tasks=true` && response.status() === 200);
  ...
  await expect(androidPage.locator(".editable-tasks > li")).toHaveCount(before.tasks.length);
  await androidPage.screenshot({ path: `${output}/06-android-chrome-history.png`, fullPage: true });
  evidence.androidBrowser = { incomingUrl: incomingEditorUrl, consumedUrl: androidPage.url(), sameOwnerGoogleRedirect: true, editorVisible: true, result: nativeHistory };
