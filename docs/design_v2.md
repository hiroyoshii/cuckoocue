...
2026-09-13 Android導線の補足（G03/G05/W10）: Androidでも「もう一度使う」は選択した完了Task本文だけを新Runへローカルコピーする。日付・優先度・完了・由来参照はクリアし、日程入力・認証は要求しない。全件完了では全選択を主操作とし、一部完了でも「内容を選んで使う」から利用できる。「再利用用に整える」は認証・同じRunの同期成功後、Webの編集draftへ直接渡す。Android直接公開は提供しない。既存の端末内Cuebookは削除せず、BQ原本の同期は未決契約を残す。
