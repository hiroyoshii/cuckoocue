...
2026-09-13 Android導線の補足（G03/G05/W10、二択へ訂正）: 全件完了後の「もう一度使う」は全完了Task本文を新Runへローカルコピーする。日付・優先度・完了・由来参照はクリアし、項目選択・日程入力・認証は要求しない。「再利用用に整える」は認証・同じRunの同期成功後、完了TaskをWebの編集draftへ直接渡す。項目の取捨選択はWeb編集内で行い、Androidに独立した選択入口やダイアログを置かない。部分完了の場合もWeb編集への入口は利用できる。Android直接公開は提供しない。既存の端末内Cuebookは削除せず、BQ原本の同期は未決契約を残す。

受渡しは既存`run_id`に`#edit_tasks=ID,ID`を付ける。fragmentには選択した既存Task IDだけを置き、本文・日付を含めずHTTPへ送らない。Webは本人限定GETの`completed_tasks=true`で完了Taskだけを取得し、選択IDの全件実在・重複なしを確認して既存editorへ渡す。履歴一覧と通常GETの全件完了条件は変更しない。選択不一致・通信失敗ではリンクを消費せず、別タスクで補完しない。成功後だけリンクを消費し、既存UID別workspaceに編集を保持する。新Entity・DBフィールド・公開操作は追加しない。
