...
Widgetには全項目を無秩序に詰め込まず、優先度、期限、リスト順、項目順を使って候補を並べます。アプリ側では「Widgetに出るCue」を確認でき、何がホーム画面へ戻ってくるかを見たうえでRun別に整えられます。完了と取り消しもWidget内で行えるため、生活の流れを中断しません。
...
下部にはRun文脈を示す短いラベルが並び、選択したRunへ表示を絞れます。項目完了後はUndoが現れます。狭いWidgetと縦長Widgetでも、同じ操作の意味を維持します。
