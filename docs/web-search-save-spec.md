...
- [ ] **UX-003: Store install links.** Set `NEXT_PUBLIC_IOS_APP_URL` and `NEXT_PUBLIC_ANDROID_APP_URL` after the App Store and Google Play listings exist. Until then, the `タスクを管理` menu shows each platform as `公開準備中` instead of emitting broken links.
